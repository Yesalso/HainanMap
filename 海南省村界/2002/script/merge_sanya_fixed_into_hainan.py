# -*- coding: utf-8 -*-
"""
把 2002/hainan/sanya/sanya_fixed.shp（三亚 19 个乡镇/区，含城芯 6 乡镇重划、
曲线化后的结果）整体替换进全省乡镇图层 Hainan_town.shp。

原理与 merge_7counties_into_hainan.py 完全一致：
  1. 以当前 Hainan_town.shp 中三亚旧乡镇（16 个）的并集作为「县界掩膜」；
  2. 把新 19 个单元逐一裁剪进掩膜，超出旧县界的部分切掉；
  3. 掩膜内未被覆盖的剩余缺口（sanya_fixed 的 76.85 m² 细缝）按共享边界
     就近并入相邻单元，保证县界严丝合缝；
  4. 三亚块仍插回它原来的位置，其余市县顺序不变；
  5. 写回前备份一次为 Hainan_town_before_sanya_fixed.shp。

用法：
  python merge_sanya_fixed_into_hainan.py
"""
import os
import sys
...
SY_SHP = os.path.join(HAINAN, "sanya", "sanya_fixed.shp")

CITY = "三亚市"
EN = "Sanya"
PREFIX = "4602"
BACKUP_TAG = "before_sanya_fixed"
...
def main():
    ...
    assert old_mask.any(), "基图中未找到三亚要素"
    old_union = sanitize(unary_union(list(hn[old_mask].geometry)).buffer(0))
    print(f"原三亚：{int(old_mask.sum())} 个单元，县界面积 {old_union.area/1e6:.4f} km²")

    layer = load_layer(SY_SHP, hn.crs, CITY, EN)
    print(f"新三亚：{len(layer)} 个单元")
    ...
    check_county(merged, mask, "三亚市", old_union)
