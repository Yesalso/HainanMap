# -*- coding: utf-8 -*-
"""
将指定市县（默认儋州、琼海）的 2002 数据整体替换进全省乡镇图层，
可选用「以原县界为准、内部用新乡镇」的方式修复边界空隙/错位。

修复原理：
  1. 以 BASE_SHP 中该市县旧乡镇的并集作为「县界掩膜」（与全省完全咬合）；
  2. 把新乡镇逐一裁剪进掩膜，超出旧县界的部分一律切掉（消除越界/重叠）；
  3. 掩膜内未被覆盖的剩余缺口（新数据比原县界小造成的空隙）按共享边界
     就近并入相邻乡镇（消除县界内的空白缝隙）。

配置 / 可复用：
  - 修改 REPLACEMENTS 即可换任意市县：{shp, city, en, prefix}
  - CLIP_TO_COUNTY_BOUNDARY=False 时不做裁剪，直接原位替换（旧文件行为）。

依赖：geopandas / pandas / shapely / pyproj
"""
import os
import shutil
...
# 全省乡镇图层（被替换的目标）。默认取备份层（含原始琼海/儋州县界），
# 保证县界与全省咬合；若备份不存在则回退到当前 Hainan_town.shp。
DEFAULT_BASE = os.path.join(DIR_2002, "hainan", "Hainan_town.shp")
BAK_BASE = os.path.join(DIR_2002, "hainan", "Hainan_town_before_danzhou_qionghai.shp")
BASE_SHP = BAK_BASE if os.path.exists(BAK_BASE) else DEFAULT_BASE

# 输出（默认直接覆盖 Hainan_town.shp）
...
REPLACEMENTS = [
    {
        "shp": os.path.join(DIR_2002, "danzhou", "danzhou2002_fixed_Albers.shp"),
        "city": "儋州市",
        "en": "Danzhou",
        "prefix": "469003",
    },
    {
        "shp": os.path.join(DIR_2002, "qionghai", "qionghai2002_corrected.shp"),
        "city": "琼海市",
        "en": "Qionghai",
        "prefix": "469002",
    },
...
def backup_once(base, tag="before_danzhou_qionghai"):
...
def main():
    ...
    if BACKUP_BEFORE:
        backup_once(OUT_SHP, tag="pre_clip")
